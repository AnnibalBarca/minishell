...
/*   Updated: 2025/08/19 16:00:36 by almeekel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	expand_string_loop(t_str_builder *sb, const char **ip, char **envp,
		int *exit_status)
{
	while (**ip)
	{
		...
		int *exit_status)
{
	...
	sb_init(&sb);
	ip = input_str;
	if (!expand_string_loop(&sb, &ip, envp, exit_status))
